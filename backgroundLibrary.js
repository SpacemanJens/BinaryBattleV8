...
//    mainCanvas.text("Distance: " + distance, mouseX, mouseY + 40);
    let sizeP = map(this.size, 0, 200, 0, 2000);
    console.log(int(sizeP));
    return distance < sizeP / 2;  // Return true if the point is inside the planet        
  }
...
//    let posY = map(flight.globalY + flight.y,0,2000, 0, 200) + this.y - this.size / 2;
    let posX = map(flight.globalX + flight.x,0,2000, 0, 200) + 200;
    let posY = map(flight.globalY + flight.y,0,2000, 0, 200) + 200;
